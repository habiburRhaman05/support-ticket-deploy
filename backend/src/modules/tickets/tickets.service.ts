...
export class TicketsService {
  async create(dto: {
    ...
    const assigneeId = await assignmentService.autoAssign(user.agencyId, dto.category);
    const stage = assigneeId ? "ACCEPTED" : "NEW";
      ...
      if (assigneeId) {
        await tx.ticketStageHistory.create({
          data: {
            ...
            stage: "ACCEPTED",
            actorId: assigneeId,
            comment: "This has been acknowledged and queued for work.",
            wasEmailed: true,
          },
        });
        ...
        await tx.notification.create({
          data: {
            ...
            message: `Ticket ${newTicket.subject} has been assigned to you.`,
          },
        });
      } else {
        // Nobody available — it lands in the unassigned queue; owners must know.
        ...
        await tx.notification.createMany({
          data: owners.map((o) => ({
            ...
            message: `No team member was available for "${newTicket.subject}" — it's in the unassigned queue.`,
          })),
        });
      }
  ...
  private canMoveTo(currentStage: string, targetStage: string, user: JwtPayload): boolean {
    ...
    if (user.role === "TEAM_MEMBER") {
      ...
      // (e.g. Pending → Working) are legitimate day-to-day flow.
      if (targetStage === "RESOLVED") return false;
