        ...
        <div className="px-6 py-5 space-y-6">
          ...
          {!isSubAccount && (
            <section className="flex items-center justify-between gap-4 bg-gray-50 rounded-xl border border-gray-100 px-4 py-3">
              ...
              {isOwner && (
                <select
                  className="text-[12.5px] border border-gray-200 rounded-lg px-2.5 py-1.5 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-black/10"
                  value={ticket.assignee?.id ?? ""}
                  onChange={(e) =>
                    assign.mutate(
                      { id: ticket.id, assigneeId: e.target.value || null },
                      {
                        onSuccess: () => toast.success(e.target.value ? "Ticket reassigned" : "Assignee removed"),
