...
function SubAccountsPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      toast.error("Could not refresh from GHL")
      refetch()
    }
  }
  ...
  return (
    <AppShell
      ...
      actions={
        ...
        >
          <RefreshCw className={cn("w-4 h-4", isFetching && "animate-spin")} /> Refresh from GHL
        </button>
      }
    >
      {isLoading ? (
                ...
                {data.locations.map((s) => (
                  <tr key={s.locationId} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      {(s.status === "NOT_CONNECTED" || s.status === "REJECTED") && s.inGhl && (
                        <button
                          type="button"
